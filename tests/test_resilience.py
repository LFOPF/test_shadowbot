...
class ResilienceTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_parse_glossary_value_keeps_non_grammar_parentheses(self):
        clean_translation, gender_note = bot.parse_glossary_value('Джет (модель XJ-9)')
        self.assertEqual(clean_translation, 'Джет (модель XJ-9)')
        self.assertIsNone(gender_note)
    ...
    async def test_get_relevant_glossary_matches_by_russian_translation(self):
        await bot.redis_client.hset('glossary:terms', mapping={'Jet': 'Джет'})
        await bot.redis_client.hset('glossary:notes', mapping={'Jet': 'женский'})
        result = await bot.get_relevant_glossary('В сцене появилась Джет и посмотрела на небо.')
        self.assertIn('Jet → Джет | note=женский', result)

    async def test_get_relevant_glossary_fallback_includes_note_entries(self):
        await bot.redis_client.hset('glossary:terms', mapping={'Saint': 'Святая'})
        await bot.redis_client.hset('glossary:notes', mapping={'Saint': 'женский'})
        result = await bot.get_relevant_glossary('Текст без явного совпадения термина.')
        self.assertIn('Saint → Святая | note=женский', result)
